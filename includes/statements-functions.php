...
class Statements_View {
	...
	function __construct() {
		$this->page_path      = get_theme_mod( 'statements_page_path' );
	...
	public function setup() {
		...
		$this->statements_data     = news_get_remote_response_json( $this->statements_response, null );
	...
	private function get_archive_data() {
		...
		$transient = null;
		$transient_expiration = intval( get_theme_mod( 'statements_archive_transient_expire' ) );
		...
		if ( $transient ) {
			...
			$data = news_get_remote_response_json( $endpoint, null );
	...
	private function get_queried_endpoint() {
		...
		$per_page = intval( get_theme_mod( 'statements_per_page' ) );
	...
	private function get_statements_response() {
		...
		return news_get_remote_response( $this->endpoint_current, 10 );
	}
